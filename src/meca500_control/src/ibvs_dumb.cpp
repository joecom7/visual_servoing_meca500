...
public:
  IBVSNode() : Node("ibvs_node")
  {
    ...
    this->declare_parameter<double>("k_collision", 0.01);
    k_collision_ = this->get_parameter("k_collision").as_double();
    ...
    this->declare_parameter<double>("collision_threshold_m", 0.05);
    collision_threshold_m_ = this->get_parameter("collision_threshold_m").as_double();

    this->declare_parameter<double>("z_min_threshold_m", 0.55);
    z_min_threshold_m_ = this->get_parameter("z_min_threshold_m").as_double();
    ...
    this->declare_parameter<std::vector<double>>(
        "joint_limits_lower", std::vector<double>{ -175.0, -70.0, -135.0, -170.0, -115.0, -36000.0 });
    ...
    RCLCPP_INFO(this->get_logger(),
                "Joint limits loaded (deg): Lower=[%.1f, %.1f, %.1f, %.1f, %.1f, %.1f], Upper=[%.1f, %.1f, %.1f, %.1f, "
                "%.1f, %.1f]",
                limits_lower_deg[0], limits_lower_deg[1], limits_lower_deg[2], limits_lower_deg[3], limits_lower_deg[4],
                limits_lower_deg[5], limits_upper_deg[0], limits_upper_deg[1], limits_upper_deg[2], limits_upper_deg[3],
                limits_upper_deg[4], limits_upper_deg[5]);

    RCLCPP_INFO(this->get_logger(), "Collision avoidance: z_min=%.3fm, threshold=%.3fm, k_collision=%.4f",
                z_min_threshold_m_, collision_threshold_m_, k_collision_);
...
private:
  ...
  // Compute collision avoidance (keep end effector z above threshold)
  void compute_collision_avoidance(const Eigen::MatrixXd& J_robot, Eigen::VectorXd& z_dot_collision,
                                   Eigen::MatrixXd& J_collision)
  {
    ...
    if (dist_from_min < collision_threshold_m_)
    {
      ...
      // Task Jacobian is the third row of robot Jacobian (z component)
      J_collision = J_robot.row(2);
      RCLCPP_INFO(this->get_logger(), "Collision avoidance active: z=%.3f, dist=%.3f, activation=%.3f", current_ee_z_,
                  dist_from_min, activation);
    }
    else
    {
      // No collision risk: return zero-row Jacobian for complete null space
  ...
  void control_loop()
  {
    ...
    if (!received_joint_state_ || !received_target_ || !received_ee_pose_)
      return;
    ...
    jacobian_client_->async_send_request(jac_req, [this](rclcpp::Client<GetJacobian>::SharedFuture future) {
      ...
      image_j_client_->async_send_request(img_req, [this, J_robot](
        ...
        // === TASK 2: COLLISION AVOIDANCE (SECONDARY) ===
        Eigen::VectorXd z_dot_collision;
        Eigen::MatrixXd J_collision;
        compute_collision_avoidance(J_robot, z_dot_collision, J_collision);

        Eigen::VectorXd q_dot_collision_task = Eigen::VectorXd::Zero(NUM_JOINTS);
        Eigen::MatrixXd P_collision = P_limit;
        ...
        if (J_collision.rows() > 0)
        {
          ...
          // Null space projector for both limit and collision
          P_collision = P_limit * (Eigen::MatrixXd::Identity(NUM_JOINTS, NUM_JOINTS) -
                                   J_collision.completeOrthogonalDecomposition().pseudoInverse() * J_collision);
        }
        ...
        // === TOTAL VELOCITY ===
        Eigen::VectorXd q_dot = q_dot_limit_task + q_dot_collision_task + q_dot_roll + q_dot_ibvs;
